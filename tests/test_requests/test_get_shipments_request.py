...
from parcelhubapi.request import BaseParcelhubApiRequest, GetShipmentsRequest


@pytest.fixture
def request_obj(mock_session):
    return GetShipmentsRequest(session=mock_session)
...
def test_url_method(mock_session, request_obj):
    request_obj = GetShipmentsRequest(mock_session)
