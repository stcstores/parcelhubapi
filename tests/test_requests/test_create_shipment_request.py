...
from parcelhubapi.request import BaseParcelhubApiRequest, CreateShipmentRequest


@pytest.fixture
def request_obj(mock_session):
    return CreateShipmentRequest(session=mock_session)
...
def test_url_attribute(request_obj):
    assert request_obj.URL == "1.0/Shipment"
...
def test_url_method(mock_session, request_obj):
    request_obj = CreateShipmentRequest(mock_session)
    assert request_obj.url() == "https://api.test.com/1.0/Shipment"
...
def test_call_method(mock_requests, request_args, request_kwargs, request_obj):
    ...
    mock_requests.request.return_value.text.side_effect = response_text
